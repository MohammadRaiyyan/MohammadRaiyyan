<h1 align="center">Hi 👋, I'm Mohammad Raiyyan</h1>

<p align="center">
  <strong>Full Stack Developer</strong><br/>
  React • Next.js • TypeScript • FastAPI • PostgreSQL • Docker
</p>

<p align="center">
Building scalable web applications with modern technologies and a strong focus on performance, user experience, and clean architecture.
</p>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Inter&weight=600&size=22&duration=3500&pause=1000&color=58A6FF&center=true&vCenter=true&width=750&lines=Full+Stack+Developer;React+%7C+Next.js+%7C+TypeScript;FastAPI+%7C+PostgreSQL+%7C+Docker;Building+Modern+Web+Applications" />
</p>


---

# 💫 About Me

- 💼 Full Stack Developer at **Wobot Intelligence Pvt. Ltd.**
- 🚀 Passionate about building scalable, performant and maintainable web applications
- ⚛️ Strong expertise in modern React ecosystem and frontend architecture
- 🏗️ Experienced in designing end-to-end applications from frontend to backend
- 🤝 Open to Open Source collaborations and interesting engineering challenges
- 🌍 Based in India

---

# ⚡ Tech Stack

### Languages

<p>
<img src="https://skillicons.dev/icons?i=js,ts,html,css,python" />
</p>

### Frontend

<p>
<img src="https://skillicons.dev/icons?i=react,nextjs,tailwind,vite,svelte,vue" />
</p>

### Backend

<p>
<img src="https://skillicons.dev/icons?i=fastapi,nodejs,postgres,docker" />
</p>

### Cloud & Tools

<p>
<img src="https://skillicons.dev/icons?i=aws,git,github,vscode,figma" />
</p>

---

# 📊 GitHub Analytics

<p align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=MohammadRaiyyan&show_icons=true&theme=tokyonight&hide_border=true&rank_icon=github"/>

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=MohammadRaiyyan&layout=compact&theme=tokyonight&hide_border=true"/>

</p>

<p align="center">

<img src="https://streak-stats.demolab.com?user=MohammadRaiyyan&theme=tokyonight&hide_border=true"/>

</p>

<p align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=MohammadRaiyyan&theme=tokyo-night&hide_border=true"/>

</p>

---

# 🚀 Featured Projects

### 📌 Project Management Platform

Modern collaborative project management platform inspired by Jira, built with a scalable full-stack architecture.

**Tech Stack**

- React
- TypeScript
- FastAPI
- PostgreSQL
- Docker
- WebSockets

---

### 📌 Real-Time Collaboration Platform

Collaborative editing platform featuring presence, synchronization, comments, notifications, and real-time updates.

**Highlights**

- WebSockets
- Optimistic UI
- Infinite Canvas
- Live Collaboration
- Offline Support

---

### 📌 AI-Powered Dashboard

Production-grade analytics dashboard with modern UI, reusable component architecture, data visualization, and high-performance rendering.

---

# 🌱 Currently Building

- 🚀 Real-Time Applications
- ⚡ Modern Full Stack Architectures
- 🧩 Design Systems
- 🔄 Collaborative Platforms
- 🤖 AI Integrated Web Applications

---

# 🌐 Connect With Me

<p align="center">

<a href="https://raiyyan.dev">🌍 Portfolio</a>

<a href="https://linkedin.com/in/m-raiyyan">LinkedIn</a>

</p>

